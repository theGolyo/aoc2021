...
def add(treeOne, treeTwo):
    ...
    c1 = clone(treeOne, newRoot)
    c2 = clone(treeTwo, newRoot)
    newRoot["left"] = c1
    newRoot["right"] = c2
    increaseDepth(c1)
    increaseDepth(c2)
...
def clone(tree, parent):
    if tree == None:
        return None
    clonedTree = createEmptyNode()
    clonedTree["parent"] = parent
    clonedTree["value"] = tree["value"]
    clonedTree["left"] = clone(tree["left"], clonedTree)
    clonedTree["right"] = clone(tree["right"], clonedTree)
    clonedTree["depth"] = tree["depth"]
    return clonedTree 
...
for t1 in trees:
    for t2 in trees:
        if t1 is not t2:
            ...
            if mag > maxMag:
                maxMag = mag
                maxTrees = (treeToString(t1), treeToString(t2))
